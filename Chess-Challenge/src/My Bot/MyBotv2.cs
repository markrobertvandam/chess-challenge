...
public class MyBotv2 : IChessBot
{
    ...
    public int eval(Board board, IEnumerable<PieceList> my_pieces, IEnumerable<PieceList> enemy_pieces)
    {
        ...
        return my_value - enemy_value + activityDiff;
    }

    public int eval_after_move(Board board, Move move)
    {
        board.MakeMove(move);
        int evaluation = eval(board, board.GetAllPieceLists().Where(c => c.IsWhitePieceList == board.IsWhiteToMove), board.GetAllPieceLists().Where(c => c.IsWhitePieceList != board.IsWhiteToMove));
    ...
    public Move ThinkGen(Board board, Timer timer, bool isBot)
    {
        ...
        foreach (Move move in allMoves)
        {
            ...
            if (isBot)
            {
                var initial_eval = eval(board, board.GetAllPieceLists().Where(c => c.IsWhitePieceList == board.IsWhiteToMove), board.GetAllPieceLists().Where(c => c.IsWhitePieceList != board.IsWhiteToMove));
